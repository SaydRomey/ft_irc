...
// Static Validator Map Initialization
std::map<CommandType, Validator::ValidatorFunc>	Validator::_validators;

Validator::Validator(void)
{
	if (_validators.empty())
	{
		_validators[PASS] = &Validator::_validatePassCommand;
		_validators[NICK] = &Validator::_validateNickCommand;
		_validators[USER] = &Validator::_validateUserCommand;
		_validators[JOIN] = &Validator::_validateJoinCommand;
		_validators[PART] = &Validator::_validatePartCommand;
		_validators[TOPIC] = &Validator::_validateTopicCommand;
		_validators[MODE] = &Validator::_validateModeCommand;
		_validators[KICK] = &Validator::_validateKickCommand;
		_validators[INVITE] = &Validator::_validateInviteCommand;
		_validators[PRIVMSG] = &Validator::_validatePrivmsgCommand;
		_validators[NOTICE] = &Validator::_validateNoticeCommand;
	}
}
...
/*
Determines the command type from a string
*/
...
Dispatches to specific validator functions
*/
bool	Validator::validate(const Message &msg, const User &user) const
{
	const t_mapStrStr	&cmd = msg.getParsedMessage();

	if (cmd.find("command") == cmd.end() || cmd.at("command").empty())
	{
		msg.setReply(ERR_UNKNOWNCOMMAND, user.getNickname(), "*");
		return (false);
	}
	const std::string	&cmd = msg.getCommand()

	CommandType	cmdType = getCommandType(msg.getCommand());

	std::map<CommandType, ValidatorFunc>::const_iterator	it = _validators.find(cmdType);
	if (it != _validators.end())
	{
		return (this->*(it->second))(msg.getParsedMessage(), user); // assuming a new function that returns the t_mapStrStr in msg...
	}
	return (msg.setReply(reply(ERR_UNKNOWNCOMMAND, user.getNickname(), msg.getCommand())));
	return (false);
}
